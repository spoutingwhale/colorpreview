...
bot = Bot(
    token=getenv("API-KEY"), default=DefaultBotProperties(parse_mode=ParseMode.HTML)
)
dp = Dispatcher()
...
async def cmd_start(message: types.Message):
    if message.from_user.language_code not in strings.keys():
        code = "en"
    else:
...
async def inline(query: types.InlineQuery):
    if query.from_user.language_code not in strings.keys():
        code = "en"
    else:
        code = query.from_user.language_code
    query_text = query.query

    if query_text == "":
        response = strings[code]["empty"]
        title = strings[code]["empty"]
    ...
    if utils.ishex(query_text) == True:
        ...
        response = utils.makeresponse(utils.generate(color), strings[code]["output"])
    elif (
        (len(query_text.split(" ")) == 3)
        and all(utils.isnum(c) for c in query_text.split(" "))
        and all(int(c) <= 255 for c in query_text.split(" "))
        and all(int(c) >= 0 for c in query_text.split(" "))
    ):
        rgb = query_text.split(" ")
        color = "%02x%02x%02x" % (int(rgb[0]), int(rgb[1]), int(rgb[2]))
        if utils.ishex(color) == True:
            title = strings[code]["title"].replace("<input>", query_text)
            response = utils.makeresponse(
                utils.generate(color),
                strings[code]["output"],
            )
        else:
    ...
    photo_url = strings["url"].replace("{}", color)
